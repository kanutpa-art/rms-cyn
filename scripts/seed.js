...
// SEED_DEMO=true → create demo tenants/bills. false (default) → empty start state.
const seedDemo     = process.env.SEED_DEMO === 'true' || process.env.NODE_ENV === 'demo';
// PASSWORD_SYNC=true → re-hash admin password every boot from env (lets owner rotate via env var).
const passwordSync = process.env.PASSWORD_SYNC === 'true';
...
if (dorm) {
  ...
  `).run(
    ...
    process.env.LIFF_ID || '',
    seedDemo ? 1 : 0  // demo skips wizard; real install runs it
  );
  dormId = r.lastInsertRowid;
  console.log(`✅ Created dormitory: "${dormName}" (id: ${dormId}, setup_completed=${seedDemo?1:0})`);
}

// ============================================================
// 2. สร้าง Admin User
// ============================================================
const existingAdmin = db.prepare('SELECT id, password_hash FROM admin_users WHERE email = ?').get(adminEmail);
let adminId;
if (existingAdmin) {
  adminId = existingAdmin.id;
  // Allow owner to rotate password by setting PASSWORD_SYNC=true env var
  if (passwordSync) {
    const newHash = bcrypt.hashSync(adminPassword, 10);
    if (newHash !== existingAdmin.password_hash) {
